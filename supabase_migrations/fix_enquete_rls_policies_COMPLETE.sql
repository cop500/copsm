-- Script COMPLET pour corriger les politiques RLS - Étape par étape

-- ÉTAPE 1: Désactiver temporairement RLS pour pouvoir supprimer les politiques
ALTER TABLE enquete_reponses DISABLE ROW LEVEL SECURITY;

-- ÉTAPE 2: Supprimer TOUTES les politiques existantes (tous les noms possibles)
...
DROP POLICY IF EXISTS "public_insert_enquete" ON enquete_reponses;
DROP POLICY IF EXISTS "admin_view_enquete" ON enquete_reponses;
DROP POLICY IF EXISTS "admin_update_enquete" ON enquete_reponses;
DROP POLICY IF EXISTS "admin_delete_enquete" ON enquete_reponses;

-- ÉTAPE 3: Réactiver RLS
ALTER TABLE enquete_reponses ENABLE ROW LEVEL SECURITY;

-- ÉTAPE 4: Créer les nouvelles politiques avec des noms uniques
